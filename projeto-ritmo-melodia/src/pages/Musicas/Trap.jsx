import React from 'react';

import '../../css/Musicas.css';

const Trap = () => {
  return (
    <div>
      <h1 className='tituloPaginaMusicas'>Alguns dos Artistas Mais Famosos do Trap</h1>

      <div> 
        <h2 className="cantores">Matuê</h2>
        <p>Matheus Brasileiro Aguiar, mais conhecido como Matuê, é um dos nomes mais influentes do cenário musical brasileiro, especialmente no universo do trap. Nascido em Fortaleza, Ceará, Matuê revolucionou o rap nacional com sua sonoridade única e letras impactantes.</p>
        <h3>História na Música</h3>
        <p>A trajetória de Matuê é marcada por uma ascensão meteórica. Influenciado pela cultura hip-hop desde jovem, ele começou a produzir suas próprias músicas ainda adolescente. Com a criação da gravadora 30PRAUM, ao lado de amigos, Matuê encontrou o ambiente ideal para desenvolver seu talento e dar vida a seus projetos. A consagração veio com o lançamento de hits como "Anos Luz" e "Máquina do Tempo", que o projetaram para o cenário nacional. Sua sonoridade experimental, que mistura elementos do trap com outros gêneros musicais, conquistou um público fiel e diversificado.</p>
        <h3>Principais Álbuns e Músicas Famosas</h3>
        <p>Alguns dos álbuns mais notáveis de Matuê incluem:</p>
        <ul>
          <li>"KIDS" (2020): Este álbum solidificou sua carreira, apresentando seu estilo único e abordagens líricas que ressoaram com o público jovem.</li>
          <li>"Máquina do Tempo" (2021): Um trabalho que mistura elementos de trap e hip-hop, continuando a trajetória de sucesso de Matuê.</li>
          <li>"333" (2024): Com as famosas "333" e "Crack com Mussilon"</li>
          <li>Algumas de suas músicas com mais destaque: "Kenny G", "Anos Luz", "Vampiro", "É Sal" e "Máquina do Tempo".</li>
          <p>Essas faixas se destacaram nas paradas de sucesso e nas plataformas de streaming, contribuindo para a popularidade crescente do artista.</p>
        </ul>
        <h3>Contribuições e Legado</h3>
        <p>Matuê é reconhecido por trazer uma nova abordagem ao trap brasileiro, combinando influências de diferentes estilos e apresentando letras que falam diretamente com a realidade dos jovens. Sua capacidade de conectar-se emocionalmente com o público, aliada a uma estética visual marcante em seus videoclipes, o tornou um ícone para muitos fãs. Além de sua música, Matuê também contribui para o fortalecimento da cena do rap e trap no Brasil, incentivando novos artistas e colaborando com diversos nomes do gênero. Seu legado se estende não apenas por suas canções, mas também por sua influência na cultura urbana contemporânea.</p>
      </div>
      <div>
        <h3>Kenny G - Matuê</h3>
          <iframe
            width="560"
            height="315"
            src="https://www.youtube.com/embed/b-PhvPKgWjY"
            title="Canção Kenny G"
            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
            allowFullScreen
          ></iframe>
      </div>

      <div> 
        <h2 className="cantores">Teto</h2>
        <p>Clériton Sávio Santos Silva, mais conhecido como Teto, é um dos nomes mais quentes do cenário musical brasileiro, especialmente no universo do trap. Nascido em Jacobina, Bahia, Teto conquistou o público com sua voz marcante, letras autênticas e um carisma inegável.</p>
        <h3>História na Música</h3>
        <p>A história de Teto é um verdadeiro fenômeno da era digital. Ainda adolescente, ele começou a compor e gravar suas músicas em um quarto, sem grandes pretensões. No entanto, o que começou como um hobby logo se transformou em uma paixão que tomaria conta de sua vida. A grande virada veio com as famosas "prévias". Teto compartilhava trechos de suas músicas nas redes sociais, e a repercussão foi instantânea. Seus fãs se apaixonaram por sua sonoridade única e por suas letras que falavam sobre o dia a dia da juventude. Rapidamente, Teto ganhou o apelido de "Rei das Prévias". A demanda por suas músicas era tão grande que, mesmo antes de lançar um álbum completo, Teto já era um dos artistas mais ouvidos no Brasil. Sua autenticidade e carisma o tornaram um dos representantes da nova geração do rap nacional.</p>
        <h3>Principais Álbuns e Músicas Famosas</h3>
        <p>Alguns das músicas mais notáveis de Teto incluem:</p>
        <ul>
          <li>"Bala e Fogo" (part. MC Cabeliho)</li>
          <li>"BBL"</li>
          <li>"Pisando nas Nuvens"</li>
          <li>"OK! OK!"</li>
          <li>"zumzumzum"</li>
          <li>"Minha Vida É Um Filme"</li>
          <li>"Flow Espacial" (part. WIU e Matuê)</li>
          <li>"Fim de Semana no Rio"</li>
          <li>"Problemas de um Milionário" (part. WIU)</li>
          <li>Vampiro" (part. Matuê e WIU)</li>
        </ul>
        <p>Essas faixas se destacaram nas paradas e ajudaram a consolidar sua presença na cena musical.</p>
        <h3>Contribuições e Legado</h3>
        <p>Teto é reconhecido como uma voz importante na nova geração do rap e trap brasileiro. Sua habilidade em misturar diferentes estilos musicais, além de suas letras que refletem a realidade de muitos jovens, contribui para sua relevância no cenário atual. Ele também tem sido uma inspiração para novos artistas, promovendo uma cultura de autenticidade e expressão. O legado de Teto se estende além de sua música, refletindo uma nova era do rap brasileiro, onde questões pessoais e sociais são abordadas com sinceridade e criatividade. Com uma base de fãs crescente, ele está moldando o futuro da música urbana no Brasil.</p>
      </div>
      <div>
        <h3>Fim de Semana no Rio - Teto</h3>
          <iframe
            width="560"
            height="315"
            src="https://www.youtube.com/embed/7AlAYttGnAg"
            title="Canção Fim de Semana no Rio"
            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
            allowFullScreen
          ></iframe>
      </div>

      <div> 
        <h2 className="cantores">Veigh</h2>
        <p>Veigh, cujo nome verdadeiro é Thiago Veigh, nasceu em 12 de setembro de 2001, em São Paulo, Brasil. Desde jovem, ele se interessou por música e começou a escrever letras e compor suas próprias canções. Veigh se destacou no cenário do rap e do trap brasileiro, utilizando suas experiências pessoais como inspiração para suas letras.</p>
        <h3>História na Música</h3>
        <p>Veigh começou a ganhar notoriedade em 2018, quando começou a lançar suas músicas nas plataformas digitais. Ele rapidamente se tornou conhecido por sua mistura de melodias cativantes e letras que abordam temas como amor, superação e a vida nas ruas. Seu estilo é caracterizado por uma produção envolvente e uma entrega lírica autêntica.</p>
        <h3>Principais Álbuns e Músicas Famosas</h3>
        <p>Alguns dos álbuns mais notáveis de Veigh incluem:</p>
        <ul>          
          <li>"Dos Prédios" (2022): Com os sucessos "Vida chique", "Perdoa Por Tudo Vida" e "Meio Pá".</li>
          <li>"Dos Prédios Deluxe" (2023): Inclue os hits "Novo Balanço" e "Engana Dizendo Que Ama" (part. TZ da Coronel).</li>
        </ul>
        <p>Outras músicas com destaque:</p>
        <ul>
          <li>"Vem Desestressar"</li>
          <li>"Olá, Moça"</li>
          <li>"Nois é Nois"</li>
          <li>"CONFISSÕES PT2"</li>
          <li>"Loucura"</li>
        </ul>
        <h3>Contribuições e Legado</h3>
        <p>Veigh é reconhecido por trazer uma nova perspectiva ao rap e trap brasileiros, explorando temas relevantes e conectando-se emocionalmente com seu público. Sua capacidade de contar histórias por meio de suas letras e sua autenticidade como artista o tornaram uma figura influente entre os jovens. Seu legado se reflete na sua contribuição para a cena musical urbana, inspirando novos artistas a serem verdadeiros em sua expressão artística. Com uma base de fãs crescente e um estilo único, Veigh está moldando o futuro do rap no Brasil, abrindo portas para a nova geração de músicos.</p>
      </div>
      <div>
        <h3>Novo Balanço - Veigh</h3>
          <iframe
            width="560"
            height="315"
            src="https://www.youtube.com/embed/fEUCYAd0KGM"
            title="Canção Novo Balanço"
            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
            allowFullScreen
          ></iframe>
      </div>

      <div> 
        <h2 className="cantores">WIU</h2>
        <p>WIU, nome artístico de Vinicius William Sales de Lima, é um rapper, cantor, compositor e produtor musical brasileiro que tem conquistado o cenário musical nacional com seu estilo único e letras marcantes. Nascido em Fortaleza, no Ceará, WIU desde cedo demonstrou paixão pelo hip-hop e pela produção musical.</p>
        <h3>História na Música</h3>
        <p>WIU começou sua jornada musical ainda adolescente, explorando programas de produção musical e compartilhando suas criações nas plataformas digitais. Sua ascensão meteórica se deu após ser descoberto por Matuê e integrado à gravadora 30PRAUM, um dos principais selos de trap no Brasil. A parceria com a 30PRAUM impulsionou a carreira de WIU, que rapidamente se tornou um dos nomes mais relevantes do trap brasileiro. Seus hits como "Felina", "Horas Iguais" e "Coração de Gelo" alcançaram o topo das paradas musicais e o tornaram um dos artistas mais ouvidos no país.</p>
        <h3>Principais Álbuns e Músicas Famosas</h3>
        <p>Alguns dos álbuns mais notáveis de WIU incluem:</p>
        <ul>          
          <li>"Manual de Como Amar Errado" (2022): Com destaque para "Coração Gelado" e "Ciumenta". </li>
          <li>"Vagabundo de Luxo" (2024): Incluem sucessos como "Rainha da Finesse" e "Eu Nao Ouço Mais Trap".</li>
        </ul>
        <p>Músicas mais tocadas do cantor:</p>
        <ul>
          <li>"Mantém" (part. Matuê)</li>
          <li>"Vampiro" (part. Matuê e Teto)</li>
          <li>"Felina" (part. MC Ryan SP)</li>
          <li>"Tenho que Me Decidir" (part. Felipe Amorim, MC PH, DG e Batidão Stronda)</li>
          <li>"Sereia do Nordeste" (part. Rogerinho e Ariel Donato)</li>
        </ul>
        <h3>Contribuições e Legado</h3>
        <p>WIU é reconhecido por sua habilidade de abordar temas profundos e pessoais, conectando-se emocionalmente com seu público. Ele representa uma nova geração de artistas que buscam ser autênticos e inovadores em sua música. Sua contribuição para o rap e trap brasileiros é significativa, pois ele traz uma nova perspectiva e uma sonoridade fresca ao gênero. Seu legado se reflete na inspiração que oferece a novos músicos e na forma como ele aborda questões relevantes para a juventude. Com uma carreira em ascensão, WIU está moldando o futuro da música urbana no Brasil, consolidando-se como uma voz importante e influente.</p>
      </div>
      <div>
        <h3>Rainha da Finesse - WIU</h3>
          <iframe
            width="560"
            height="315"
            src="https://www.youtube.com/embed/dC6NPbqJV6k"
            title="Canção Rainha da Finesse"
            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
            allowFullScreen
          ></iframe>
      </div>

      <div> 
        <h2 className="cantores">Chefin</h2>
        <p>Chefin, cujo nome verdadeiro é Natanael Cauã Almeida, nasceu em 14 de setembro de 2004, no Rio de Janeiro, Brasil. Desde jovem, Chefin se interessou pela música, especialmente pelo rap e trap. Ele começou a escrever suas próprias letras e a produzir suas canções, buscando expressar suas vivências e sentimentos por meio da música.</p>
        <h3>História na Música</h3>
        <p>Chefin começou a ganhar destaque no cenário musical em 2018, quando lançou suas primeiras músicas nas plataformas digitais. Sua habilidade em mesclar melodias cativantes com letras que abordam temas como amor, superação e a vida nas ruas rapidamente conquistou uma base de fãs. Com um estilo autêntico e uma presença marcante, ele se tornou uma figura importante na nova geração do rap brasileiro.</p>
        <h3>Principais Álbuns e Músicas Famosas</h3>
        <p>Alguns dos álbuns mais notáveis de Chefin incluem:</p>
        <ul>
          <li>"O Mais Novo Romântico" (2023): Inclue hits como "Do Seu Lado (part. Oruam)" e "Primeira Dama".</li>
          <li>"A Nova Era" (2022): Álbum que apresenta faixas como "A Nova Era (part. Racovi e Bren)" e "Tropa do Mais Novo (part. Vulgo FK)"</li>
        </ul>
        <p>Músicas mais tocadas:</p>
        <ul>
          <li>"10 Carros"</li>
          <li>"A Essência (part. Bren)"</li>
          <li>"Tropa do Flamengo"</li>
          <li>"Deus É o Meu Guia"</li>
        </ul>
        <h3>Contribuições e Legado</h3>
        <p>Chefin é reconhecido por sua autenticidade e pela maneira como aborda temas relevantes para os jovens, conectando-se emocionalmente com seu público. Ele é um representante da nova onda do rap e trap brasileiro, trazendo uma mistura de influências e um estilo fresco ao gênero. Seu legado se reflete na inspiração que proporciona a novos artistas e na maneira como ele eleva a cena musical urbana. Com uma carreira em ascensão e uma base de fãs leal, Chefin está moldando o futuro da música no Brasil, solidificando sua posição como uma voz importante e inovadora.</p>
      </div>
      <div>
        <h3>Tropa do Mais Novo - Chefin (part. Vulgo FK)</h3>
          <iframe
            width="560"
            height="315"
            src="https://www.youtube.com/embed/Ww056PGMT8Y"
            title="Canção Tropa do Mais Novo"
            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
            allowFullScreen
          ></iframe>
      </div>

      <div> 
        <h2 className="cantores">MC Cabelinho </h2>
        <p>MC Cabelinho, cujo nome verdadeiro é Victor Hugo Oliveira, nasceu em 28 de janeiro de 1996, na cidade do Rio de Janeiro, Brasil. Crescendo em um ambiente marcado pela cultura de rua e pelo funk carioca, ele começou a se interessar pela música desde jovem. A carreira de MC Cabelinho decolou quando ele começou a se apresentar em bailes e eventos locais, onde rapidamente ganhou notoriedade.</p>
        <h3>História na Música</h3>
        <p>MC Cabelinho se destacou no cenário do funk brasileiro em meados de 2017, quando lançou suas primeiras músicas e videoclipes. Com letras que abordam temas do cotidiano, amor, e as experiências da vida nas comunidades cariocas, ele conquistou um público fiel. Seu estilo autêntico e suas letras envolventes o ajudaram a se firmar como um dos principais nomes do funk na nova geração.</p>
        <h3>Principais Álbuns e Músicas Famosas</h3>
        <p>Alguns dos álbuns mais notáveis de MC Cabelinho incluem:</p>
        <ul>
          <li>"Minha Raíz" (2018): Apresenta "Minha Raiz".</li>
          <li>"Ainda" (2020): Com destaque para "Reflexo (part. BK)" e "Favela Venceu (part. MC Hariel)".</li>
          <li>"LITTLE HAIR" (2021): Contém as famosas "ESSÊNCIA DE CRIA", "X1", "LITTLE HAIR" e "Eu Sou o Trem".</li>
          <li>"LITTLE LOVE" (2022): Inclue músicas como "INTENÇÃO", "TANTO FAZ", "NOSSA MÚSICA" e "MINHA CURA".</li>
          <li>Outras músicas em destaque: "FOGO E GASOLINA", "OH FÉ (part. ARIEL DONATO)",  </li>
        </ul>
        <h3>Contribuições e Legado</h3>
        <p>MC Cabelinho é reconhecido por sua habilidade de contar histórias através de suas letras, abordando temas relevantes e conectando-se com o público jovem. Ele representa uma nova era do funk carioca, que busca misturar tradição e inovação. Seu legado se reflete na maneira como ele eleva a cena do funk, inspirando novos artistas a se expressarem e a explorarem suas vivências. Com uma base de fãs crescente e uma carreira em ascensão, MC Cabelinho está moldando o futuro do funk no Brasil, consolidando-se como uma voz importante e influente no gênero.</p>
      </div>
      <div>
        <h3>X1 - MC Cabelinho</h3>
          <iframe
            width="560"
            height="315"
            src="https://www.youtube.com/embed/zgdQsLH7sDw"
            title="Canção X1"
            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
            allowFullScreen
          ></iframe>
      </div>

      <div> 
        <h2 className="cantores">Tasha & Tracie</h2>
        <p>Tasha & Tracie são uma dupla de cantoras e compositoras brasileiras, formadas por Tasha (Tasha Okereke) e Tracie (Tracie Okereke), que ganharam destaque na cena musical brasileira com seu estilo autêntico e envolvente. Elas são conhecidas por sua fusão de ritmos que inclui funk, rap e R&B, trazendo uma nova perspectiva ao cenário da música urbana.</p>
        <h3>História na Música</h3>
        <p>Tasha & Tracie começaram suas carreiras como influenciadoras digitais e, em 2020, decidiram se unir musicalmente. Elas rapidamente chamaram a atenção do público com suas letras que abordam temas de empoderamento feminino, relacionamentos e a realidade das mulheres nas comunidades. A parceria foi um sucesso, e suas músicas começaram a ganhar popularidade nas plataformas de streaming e redes sociais.</p>
        <h3>Principais Álbuns e Músicas Famosas</h3>
        <p>Alguns dos álbuns mais notáveis de Tasha & Tracie incluem:</p>
        <ul>
          <li>"Diretoria (2021): Inclue "ROUFF (Prod. CESRV)" e "Amarrou feat. Veigh (Prod. DJ MF e Pizzol)".</li>
        </ul>
        <p>top 5 mais tocadas:</p>
        <ul>
          <li>"Tang (part. Kyan)"</li>
          <li>"Diretoria"</li>
          <li>"Desce Licor"</li>
          <li>"Arrume-se Comigo"</li>
          <li>"Salve"</li>
        </ul>
        <h3>Contribuições e Legado</h3>
        <p>Tasha & Tracie são reconhecidas por sua contribuição ao empoderamento feminino na música, utilizando suas vozes para falar sobre questões importantes e inspirar outras mulheres. Elas têm se posicionado como ícones na nova geração de artistas brasileiras, promovendo uma mensagem de força, união e liberdade de expressão. O legado da dupla se reflete na forma como têm influenciado novas artistas e na maneira como abordam temas relevantes, desafiando estereótipos e quebrando barreiras na indústria musical. Com uma base de fãs crescente e uma carreira promissora, Tasha & Tracie estão moldando o futuro da música urbana no Brasil.</p>
      </div>
      <div>
        <h3>Diretoria - Tasha & Tracie</h3>
          <iframe
            width="560"
            height="315"
            src="https://www.youtube.com/embed/nZfVmuVzJ3I"
            title="Canção Diretoria"
            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
            allowFullScreen
          ></iframe>
      </div>
    
      <div> 
        <h2 className="cantores">L7nnon</h2>
        <p>L7nnon, cujo nome verdadeiro é Lennon Barbosa, nasceu em 30 de março de 1994, no Rio de Janeiro, Brasil. Desde cedo, ele se interessou pela música, especialmente pelo rap e pelo trap. Com uma trajetória marcada por desafios pessoais, L7nnon começou a escrever letras como uma forma de se expressar e contar suas experiências de vida.</p>
        <h3>História na Música</h3>
        <p>L7nnon começou a ganhar notoriedade em 2018, quando lançou suas primeiras músicas nas plataformas digitais. Seu estilo mistura rap, trap e elementos do R&B, destacando-se pela profundidade de suas letras e por suas melodias envolventes. Através de sua música, ele aborda temas como amor, superação e a realidade das periferias, conquistando uma base de fãs leal.</p>
        <h3>Principais Álbuns e Músicas Famosas</h3>
        <p>Alguns dos álbuns mais notáveis de L7nnon incluem:</p>
        <ul>
          <li>"Podium" (2019): Inclue as famosas "Sorrisos (part. Lourena)", "Podium" e "Ninguém (part. MV Bill)".</li>
          <li>"Hip Hop Rare" (2020): Apresenta com destaque "Algumas Frases", "Perdição", "Inabalável (part. MC Hariel)" e "Gratidão".</li>
          <li>"Me Espera" (2023): Conhecido por "Bonita (part. Camila Zasoul)" e "Tudo ou Nada (Part. OIK)".</li>
          <li>"IRRASTREÁVEL" (2024): Contém "Maldade Na Mente", "Né Segredo (part. BK')", "Vamo Marolar (part. MC Cabelinho)" e "Posturado e Calmo (part. Anitta)".</li>
          <li>Outras músicas mais tocadas: "Abençoado", "Foto da Unha (part. Veigh)", "Lembranças" e "Pique Modelo (part. Rare G e Chris MC)".</li>
        </ul>
        <h3>Contribuições e Legado</h3>
        <p>L7nnon é reconhecido por sua capacidade de conectar-se emocionalmente com o público por meio de suas letras sinceras e introspectivas. Ele representa uma nova geração de artistas que buscam ser autênticos em sua expressão musical, e sua contribuição para a cena do rap e trap brasileiro é significativa. Seu legado se reflete na inspiração que oferece a novos músicos e na forma como aborda questões relevantes para a juventude, promovendo uma mensagem de esperança e resiliência. Com uma carreira em ascensão, L7nnon está moldando o futuro da música urbana no Brasil, solidificando-se como uma voz importante e influente no gênero.</p>
      </div>
      <div>
        <h3>Perdição - L7nnon</h3>
          <iframe
            width="560"
            height="315"
            src="https://www.youtube.com/embed/uzSme3PRhZ0"
            title="Canção Perdição"
            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
            allowFullScreen
          ></iframe>
      </div>

      <div> 
        <h2 className="cantores">Xamã</h2>
        <p>Xamã, cujo nome verdadeiro é Geizon Carlos (também conhecido como Xamã da Silva), nasceu em 30 de outubro de 1989, no Rio de Janeiro, Brasil. Desde jovem, ele foi influenciado pela cultura do hip-hop e pela música brasileira, começando a se interessar pela produção musical e pela escrita de letras. Sua jornada começou em batalhas de freestyle e ele rapidamente se destacou no cenário do rap.</p>
        <h3>História na Música</h3>
        <p>Xamã começou a ganhar notoriedade em 2019, quando lançou suas primeiras músicas e colaborou com outros artistas emergentes. Ele é conhecido por suas letras poéticas e reflexivas, que abordam temas como a vida nas favelas, amor, superação e espiritualidade. Seu estilo mistura rap, trap e influências da música brasileira, criando um som único e envolvente.</p>
        <h3>Principais Álbuns e Músicas Famosas</h3>
        <p>Alguns dos álbuns mais notáveis de Xamã incluem:</p>
        <ul>
          <li>"Pecado Capital" (2018): Com destaque para "Luxúria (part. Bagua Records, Neo Beats e Matuê)", "A Bela e a Fera 2" e "44 Problems". </li>
          <li>"O Iluminado" (2019): Contém "American Pie (part. Orochi)", "Uma Linda Mulher (part. Luccas Carlos)" e "Central do Brasil (4:20)".</li>
          <li>"Zodíaco" (2020): Com as famosas "Leão (part. Marília Mendonça)", "Sagitário (part. Gustah)", "De novo (part. Agnes Nunes)" e "Peixes (part. Portugal no Beat e Xavi)".</li>
          <li>"O Último Romântico Online" (2023): Apresenta "Puto de Luxo (part. Vulgo FK).</li>
          <li>Outras músicas mais tocadas: "Malvadão 3", "Deixe-me Ir", "Monalisa", "A Bela e a Fera" e "Escorpião (part. Agnes Nunes)". </li>
        </ul>
        <h3>Contribuições e Legado</h3>
        <p>Xamã é reconhecido por sua capacidade de contar histórias através de suas letras, conectando-se emocionalmente com o público. Ele se destaca como uma voz importante na nova geração do rap brasileiro, promovendo uma mensagem de esperança e empoderamento. Seu legado se reflete na forma como ele inspira novos artistas a se expressarem e a abordarem questões relevantes por meio da música. Com uma carreira em ascensão e uma base de fãs crescente, Xamã está moldando o futuro do rap no Brasil, consolidando-se como uma figura influente no gênero.</p>
      </div>
      <div>
        <h3>Sagitário - Xamã</h3>
          <iframe
            width="560"
            height="315"
            src="https://www.youtube.com/embed/tRiS9dbw2bM"
            title="Canção Sagitário"
            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
            allowFullScreen
          ></iframe>
      </div>

      <div> 
        <h2 className="cantores">Slipmami</h2>
        <p>Slipmami, cujo nome verdadeiro é Yasmin Pinto da Silva,, nasceu em 3 de junho de 2000, no Rio de Janeiro, Brasil. Desde jovem, ela se interessou pela música e pela cultura do hip-hop, especialmente pelo rap. Com uma forte presença nas redes sociais, Slipmami começou a se destacar por sua autenticidade e letras que refletem suas experiências pessoais e a realidade das comunidades cariocas.</p>
        <h3>História na Música</h3>
        <p>Slipmami ganhou notoriedade no cenário do rap brasileiro em 2019, quando lançou suas primeiras músicas nas plataformas digitais. Sua mistura de rap e funk, combinada com letras empoderadoras e envolventes, rapidamente conquistou um público fiel. Ela se destaca pela sua abordagem direta e por trazer à tona questões como empoderamento feminino, amor e desafios da vida urbana.</p>
        <h3>Principais Álbuns e Músicas Famosas</h3>
        <p>Alguns dos álbuns mais notáveis de Slipmami incluem:</p>
        <ul>
          <li>"Gostosa Posturada" (2021): Apresenta "Potranca" e "Gostosa Posturada".</li>
          <li>"Malvatrem" (2023): Inclue as faixas "8x5 (part. Yung Nobre)" e "Malvatrem (part. Leo Justi, Dorly)".</li>
          <li>"Até Aqui, Slip Nos Ajudou" (2024): Com canções como "E.P.A.M." e "Eles São Fracos".</li>
          <li>Outras músicas mais tocadas: "Big Momma Freestyle" e "Oompa Loompa (part. LARINHX)".</li>
        </ul>
        <h3>Contribuições e Legado</h3>
        <p>Slipmami é reconhecida por sua habilidade de abordar questões relevantes para a juventude e por sua autenticidade como artista. Ela se tornou uma voz importante na cena do rap e funk brasileiros, inspirando outras mulheres a se expressarem e a se posicionarem no mundo da música. Seu legado se reflete na forma como desafia estereótipos e promove uma mensagem de empoderamento feminino. Com uma carreira em ascensão e uma base de fãs crescente, Slipmami está moldando o futuro da música urbana no Brasil, consolidando-se como uma influente artista da nova geração.</p>
      </div>
      <div>
        <h3> Oompa Loompa - Slipmami</h3>
          <iframe
            width="560"
            height="315"
            src="https://www.youtube.com/embed/bGdlXTZc7F0"
            title="Canção Oompa Loompa"
            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
            allowFullScreen
          ></iframe>
      </div>
      
    </div>
  )
}

export default Trap
